...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   ...
   a = 0; b=1;  c=0; d=1; n = 10;  tfinal = 0.5;
...
%-- Initial condition:
   ...
   for i=1:m+1,
      for j=1:m+1,
         u1(i,j) = exp(-t)* sin(pi*x(i))* sin(pi*y(j)); 
      end
   end
...
for i=1:m+1,                              % Boundary condition.
  u2(i,1) = exp(-t2)* sin(pi*x(i))* sin(pi*y(1)); 
  u2(i,n+1) = exp(-t2)* sin(pi*x(i))* sin(pi*y(n+1)); 
  u2(1,i) = exp(-t2)* sin(pi*x(1))* sin(pi*y(i)); 
  u2(m+1,i) = exp(-t2)* sin(pi*x(m+1))* sin(pi*y(i)); 
end
...
for j = 2:n,                             % Look for fixed y(j) 
   ...
   for i=2:m,
      b(i-1) = (u1(i,j-1) -2*u1(i,j) + u1(i,j+1))/h1 + ...
		exp(-t2)*sin(pi*x(i))*sin(pi*y(j))*(2*pi^2-1) + 2*u1(i,j)/dt;
      if i == 2
        b(i-1) = b(i-1) + (exp(-t2)* sin(pi*x(i-1))* sin(pi*y(j)))/h1;
        A(i-1,i) = -1/h1;
      else
	if i==m
          b(i-1) = b(i-1) + (exp(-t2)* sin(pi*x(i+1))* sin(pi*y(j)))/h1;
...
for i=1:m+1,                                % Boundary condition
  u1(i,1) = exp(-t1)* sin(pi*x(i))* sin(pi*y(1));
  u1(i,n+1) = exp(-t1)* sin(pi*x(i))* sin(pi*y(m+1));
  u1(1,i) = exp(-t1)* sin(pi*x(1))* sin(pi*y(i));
  u1(m+1,i) = exp(-t1)* sin(pi*x(m+1))* sin(pi*y(i));
end
...
for i = 2:m,
   ...
   for j=2:n,
      b(j-1) = (u2(i-1,j) -2*u2(i,j) + u2(i+1,j))/h1 + ...
                exp(-t2)*sin(pi*x(i))*sin(pi*y(j))*(2*pi^2-1)    + 2*u2(i,j)/dt;
      if j == 2
        b(j-1) = b(j-1) + (exp(-t1)* sin(pi*x(i))* sin(pi*y(j-1)))/h1;
        A(j-1,j) = -1/h1;
      else
        if j==n
          b(j-1) = b(j-1) + (exp(-t1)* sin(pi*x(i))* sin(pi*y(j+1)))/h1;
...
%----------- Data analysis ----------------------------------
  ...
  for i=1:m+1,
    for j=1:n+1,
       ue(i,j) = exp(-tfinal)* sin(pi*x(i))* sin(pi*y(j));
    end
  end
